const { SlashCommandBuilder, Client, ChatInputCommandInteraction, EmbedBuilder, StringSelectMenuBuilder, ActionRowBuilder } = require('discord.js');
...
module.exports = {
    ...
    execute: async (client, interaction) => {
        ...
        const button = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('change-to-banner').setLabel('Banner').setEmoji(changeEmoji).setStyle('Primary').setDisabled(!user.banner), new ButtonBuilder().setURL(user.displayAvatarURL({ size: 1024 })).setLabel('Avatar Link').setStyle('Link'))
        const size_menu = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().addOptions(sizes.map(size => ({ label: `${size}x${size}`, value: size.toString() }))).setCustomId('size_menu').setPlaceholder(`${sizeLanguage}: 1024x1024`).setMaxValues(1).setDisabled(!user.avatar))
        const format_menu = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().addOptions(formats.map(format => ({ label: format.toUpperCase(), value: format }))).setCustomId('format_menu').setPlaceholder(`${formatLanguage}: ${user.displayAvatarURL().includes('.gif') ? 'GIF' : 'WEBP'}`).setMaxValues(1).setDisabled(!user.avatar))
        const change_menu = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().addOptions({ label: 'Server', value: 'server', emoji: changeEmoji }).setCustomId('change_menu').setPlaceholder(`${displayAvatarLanguage}: User`).setMaxValues(1))
