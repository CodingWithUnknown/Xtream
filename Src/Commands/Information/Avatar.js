...
module.exports = {
    ...
    execute: async (client, interaction) => {
        ...
        const msg = await interaction.editReply({ embeds: [embeds], components: [size_menu, format_menu, change_menu, button] });

        let collector = msg.createMessageComponentCollector({ idle: 3e5 });

        collector.on('collect', async (editor) => {
                ...
                if (type === 'avatar') {
                    ...
                    if (editor.customId === 'change-to-banner') {
                        ...
                        await format_menu.components[0]
                            .setOptions(formats1.map((format) => ({ label: format.toUpperCase(), value: format })))
                            .setPlaceholder(`Format: ${format.toUpperCase()}`)
                        await button.components[1]
                            .setURL(user.bannerURL({ size: Number(size), extension: format, forceStatic: true }))
                            .setLabel('Banner Link')
                        await change_menu.components[0]
                            .setPlaceholder(`Displayed Banner: User`)
                            .setDisabled(true)
                        embeds.setFields(
                            {
                                name: `General`, value: [
                        ...
                        if (editor.values[0] === 'server') {
                            ...
                            await change_menu.components[0]
                                ...
                                .setPlaceholder(` Displayed Avatar: Server`)
                            await format_menu.components[0]
                                .setOptions(formats1.map(format => ({ label: format.toUpperCase(), value: format })))
                                .setPlaceholder(`Format: ${format.toUpperCase()}`)
                            await button.components[1]
                                .setURL(member.displayAvatarURL({ size: Number(size), extension: format, forceStatic: true }))
                            await editor.editReply({ embeds: [embeds], components: [size_menu, format_menu, change_menu, button] })
                            ...
                            await change_menu.components[0]
                                ...
                                .setPlaceholder(` Displayed Avatar: User`)
                            await format_menu.components[0]
                                .setOptions(formats2.map(format => ({ label: format.toUpperCase(), value: format })))
                                .setPlaceholder(`Format: ${format.toUpperCase()}`)
                            await button.components[1]
                                .setURL(user.avatarURL({ size: Number(size), extension: format, forceStatic: true }))
                            await editor.editReply({ embeds: [embeds], components: [size_menu, format_menu, change_menu, button] })
                        }
                        ...
                        if (change === 'user') {
                            ...
                            await button.components[1]
                                .setURL(user.displayAvatarURL({ size: Number(size), extension: format, forceStatic: true }))
                            await size_menu.components[0]
                                .setPlaceholder(`Size: ${size}x${size}`)
                            await editor.editReply({ embeds: [embeds], components: [size_menu, format_menu, change_menu, button] })
                    ...
                    if (editor.customId === 'change-to-avatar') {
                        ...
                        await button.components[0]
                            ...
                            .setEmoji('🔁')
                        await format_menu.components[0]
                            .setOptions(formats3.map(format => ({ label: format.toUpperCase(), value: format })))
                            .setPlaceholder(`Format: ${format.toUpperCase()}`)
                        await button.components[1]
                            .setURL(user.displayAvatarURL({ size: Number(size), extension: format, forceStatic: true }))
                            .setLabel('Avatar Link')
        ...
        collector.on('end', async () => {
            change_menu.components[0]
                .setDisabled(true)
            size_menu.components[0]
                .setDisabled(true)
            format_menu.components[0]
                .setDisabled(true)
            button.components[0]
                .setDisabled(true)

            await interaction.editReply({ components: [size_menu, format_menu, change_menu, button] });
        })
    }
}